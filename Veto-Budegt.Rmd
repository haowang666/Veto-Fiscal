...
**Key words**:  budget policy, institutional frictions, punctuations \footnote{Replication files available on author's Github account: https://github.com/haowang666}
...
This project test the budget stability hypothesis with a newly-compiled dataset: GSRE (Global State Revenues and Expenditures dataset). 


Government policy making involves negotiations of different branches and institutions. In democratic countries, to pass a policy modification requires the majority rule. This requirement makes the status quo at an advantageous position: any further move has to beat the status quo with a majority of votes. This instituional constraint is featured in the 'veto-player' theory [@Tsebelis2002]. 
...
Both punctuated equilibrium theory and veto player theory emphasize the importance of veto player/frictions. They share many similarities in predicting the overall budget changes. Veto player theory predicts that        
...
ggplot(data = test) +
  ...
  xlim(-5, 5) +
  xlab("") +
  annotate("text", x = -4, y = 0.55, label = "Red Line: LK = 0.44") +
  annotate("text", x = -4, y = 0.5, label = "Blue Line: LK = 0.12") 
...
group1 <- c("Brazil", "Mexico", "Nicaragua", "Colombia")
mydata2 <- filter(mydata1, country %in% group1 )



ggplot(data = mydata2) +
    geom_point(aes(x = e_h_polcon5, y = stability_1)) +
