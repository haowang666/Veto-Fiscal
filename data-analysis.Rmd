...
library(DataCombine)
attach(mydata)
mydata <- mydata[order(cowcode, year), ]
...
mydata <- 
    ...
    mutate(lag.expend_security_EXP = lag(expend_security_EXP, 1), 
           ...
           lag.subpentrans_EXP = lag(subpentrans_EXP, 1),
...
mydata1 <- mydata[c("country", "year", "expend_security_EXP", "expenddefence_EXP",
                   "exp_public_order_EXP", "wagessalaries_EXP", "subpentrans_EXP",
...
set.seed(1)
library(Amelia)
a.out <- amelia(mydata1, idvars = c("year", "country"), m=1)
missmap(a.out)
write.amelia(a.out, impute, separate = TRUE, format = "csv")
impute <- a.out[[1]][[1]]
```
